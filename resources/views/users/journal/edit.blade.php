                    ...
                    <div class="card-body">
                        ...
                        <div class="row">
                            <div class="col-md-4">
                                ...
                                <input type="text" name="document_no" class="form-control" value="{{$journal->document_no}}" required>
                            </div>
                            <div class="col-md-4">
                            ...
                            <div class="col-md-4">
                                ...
                                <input type="text" name="reference" class="form-control" value="{{$journal->reference}}">
                            </div>
                            <div class="col-md-8 mt-4">
                                <label for="">Description</label>
                                <textarea name="description" class="form-control" cols="30" rows="10" required>{{$journal->description}}</textarea>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <table class="table table-hover">
                            ...
                            <tbody>
                                @foreach($journal->journal_items as $i => $journal_item)
                                    <tr id="tr_{{$i}}">
                                        <td>
                                            <select name="coa_id[]" class="form-control select2" required>
                                                <option value=""></option>
                                                @foreach($coas as $coa)
                                                    <option value="{{$coa->id}}" @if($coa->id == $journal_item->coa_id) selected="selected" @endif >{{$coa->coa}} - {{$coa->name}}</option>
                                                @endforeach
                                            </select>
                                        </td>
                                        <td> <input type="number" name="debit[]" id="debit_{{$i}}" class="form-control debit" value="{{$journal_item->debit}}" onchange="changeAmount('{{$i}}')"> </td>
                                        <td> <input type="number" name="credit[]" id="credit_{{$i}}" class="form-control credit" value="{{$journal_item->credit}}" onchange="changeAmount('{{$i}}')"> </td>
                                        <td>
                                            <textarea name="description_item[]" class="form-control" cols="30" rows="10"></textarea>
                                        </td>
                                        <td>
                                            <div id="amount_{{$i}}" class="amounts">{{abs($journal_item->debit - $journal_item->credit)}}</div>
                                        </td>
                                        <td> <label class="btn btn-sm btn-danger" onclick="deleteData('{{$i}}')"> <i class="fa fa-trash"></i> </label> </td>
                                    </tr>
                                @endforeach
                            </tbody>
                            <tbody id="addData">
                            ...
                            <tbody>
                                <tr>
                                    ...
                                    <td id="total_credit"> {{$journal->journal_items ? $journal->journal_items->sum('credit') : ''}} </td>
                                </tr>
                                <tr>
                                    <td colspan="3"></td>
                                    <td class="text-right"><b> Total Debit (Rp.) : </b></td>
                                    <td id="total_debit"> {{$journal->journal_items ? $journal->journal_items->sum('debit') : ''}} </td>
                                </tr>
                            </tbody>
                        </table>
                        <div class="text-right">
                            <button class="btn btn-success">Edit Journal</button>
                        </div>
                    </div>
                </div>
            </div>
